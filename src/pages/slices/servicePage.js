      ...
      >
        ...
        <div className='flex items-center justify-center w-8 h-8'>
          <ChevronDown className={`transition-transform duration-200 ${
            isOpen ? 'transform rotate-180' : ''}`} size={18} />
        </div>
      </button>
      {isOpen && (
  ...
  const content = (
    <>
      ...
      {(page.popular && page.popular.length > 0) &&
        <section>
          <div className='max-w-7xl mx-auto px-4 lg:px-8 overflow-x-auto py-12'>
            <div className='mb-4'>
              <h2 className='font-semibold text-2xl'>Most Popular in {page.title}</h2>
            </div>
            <div className='flex gap-x-4'>
              { page.popular.map((pop, idx) => (
                <div key={idx} className='flex items-center rounded-md'>
                  <a href={pop.link}>
                    <img className='w-8 h-8 rounded' src={pop.thumb} alt='' />
                    <span className='ml-3'>{pop.title}</span>
                  </a>
                </div>
              ))}
            </div>
            ...
            <div className='grid grid-cols-2 gap-8 lg:grid-cols-4'>
              { page.cats.map((cat, idx) => (
                <div key={idx}>
                  ...
                  <div><h3 className='my-2 text-gray-800 dark:text-gray-200'>{cat.title}</h3></div>
                  <div>
                    { cat.subs.map((sub, id) => (
                      <div key={id} className='my-3 group text-base'>
                        <a
                          href={sub.link}
            ...
            <div className='grid md:grid-cols-3 gap-x-8'>
              { page.guides.map((guide, idx) => (
                <div key={idx}>
                  <a href={guide.link}>
                    <div>
                      <img className='rounded-lg h-full' src={guide.image} alt='' />
                    </div>
                    <p className='my-3'>{guide.title}</p>
                  </a>
                </div>
              ))}
            </div>
